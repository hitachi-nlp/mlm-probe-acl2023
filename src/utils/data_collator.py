...
class DataCollatorForFirstCharPrediction:
    """Data collator used for masked first character prediction."""
    ...
    def get_label(self, val:list):
        return list(map(lambda x: self.id_to_label[x], val))
    ...
    def mask_tokens(self, input_ids):
        ...
        # create labels
        masked_word_labels = [
            self.get_label(val) for val in input_ids.tolist()
        ]
        masked_word_labels = torch.tensor(masked_word_labels, dtype=torch.long)
...
class DataCollatorForEndCharPrediction:
    """Data collator used for masked end character prediction."""
    ...
    def __init__(self, 
        ...
        for token, input_id in vocab_dict.items():
            token = ''.join(list(reversed(token)))
            if punctuation_pattern.match(token) is not None:
                self.id_to_label[input_id] = 26 
                continue
            
            if digit_pattern.match(token) is not None:
                self.id_to_label[input_id] = 27
                continue
            
            if len(token) == 1:
                label = char_to_label.get(token.lower(), None)
                if label is None:
                    self.id_to_label[input_id] = 28 # exception
                else:
                    self.id_to_label[input_id] = label
            elif token[0] == "Ġ":
                label = char_to_label.get(token[1].lower(), None)
                if label is None:
                    self.id_to_label[input_id] = 28 # exception
    ...
    def __call__(self, examples):
        ...
        # mask tokens and create word masks (labels)
        masked_input_ids, masked_word_labels = self._mask_tokens(batch["input_ids"])
    ...
    def _mask_tokens(self, input_ids):
        ...
        # create labels
        masked_word_labels = [
            self._get_label(val) for val in input_ids.tolist()
        ]
        masked_word_labels = torch.tensor(masked_word_labels, dtype=torch.long)
...
class DataCollatorForNCharsPrediction:
    """Data collator used for masked N-characters prediction."""
    ...
    def __init__(self, 
        ...
        # create label references - pt 1
        # - collect all the candidates and assign a label
        vocab_dict = tokenizer.get_vocab()
        char_list = []
        for token in vocab_dict.keys():
            if len(token) == 1:
                char_list.append(token[0])
            else:
                if lang_type == 'en':
                    if token[0] == "Ġ": # w/ blank char
                        if len(token) <= (num_chars + 1): # blank char + single char
                            char_list.append(token[1:])
                        else:
                            char_list.append(token[1:num_chars+1])
                    else: # w/o blank char
                        if len(token) <= num_chars:
                            char_list.append(token)
                        else:
                            char_list.append(token[:num_chars])
                elif lang_type in ('ja'):
                    if len(token) <= num_chars:
                        char_list.append(token)
                    else:
                        char_list.append(token[:num_chars])
                else:
                    raise ValueError('No such a lang_type!')
        self.chars_to_label = {chars: index for index, chars in enumerate(set(char_list))}
        
        # create label references - pt 2
        # - assign a lebel to each token id
        self.id_to_label = {}
        for token, token_id in vocab_dict.items():
            # get a query
            if len(token) == 1:
                ...
                if lang_type == 'en':
                    if token[0] == "Ġ": # w/ blank char
                        if len(token) <= (num_chars + 1):
                            query = token[1:]
                        else:
                            query = token[1:num_chars+1]
                    else:
                        if len(token) <= num_chars:
                            query = token
                        else:
                            query = token[:num_chars]
                elif lang_type in ('ja'):
                    if len(token) <= num_chars:
                        query = token
                    else:
            ...
            # assign a label
            self.id_to_label[token_id] = self.chars_to_label[query]
    
    
    def get_num_class(self) -> int:
        """Return the number of classification classes."""
        return len(set(self.id_to_label.values()))
...
class DataCollatorForTailNCharsPrediction:
    """Data collator used for masked tail N-characters prediction."""
    ...
    def __init__(self, 
        ...
        chars_to_label = {chars: index for index, chars in enumerate(set(char_list))}
        ...
        for token, token_id in vocab_dict.items():
            ...
            # assign a label
            self.id_to_label[token_id] = chars_to_label[query]
